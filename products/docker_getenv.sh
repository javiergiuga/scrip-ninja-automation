#!/bin/bash
# table utils
source ./scrip-ninja-automation/products/printTable.sh
# check version
if [ -z "$VERSION" ]
      then
      VERSION=`jq -r '.version' ./products/package.json`
...
AUTOMATION_SHORT=$(cd scrip-ninja-automation/products;git rev-parse --short HEAD)
AUTOMATION_BRANCH_NAME=$(cd scrip-ninja-automation/products;git rev-parse --abbrev-ref HEAD)
...
DOCKER_IMAGENAME,$REGISTRY/$NAMEP:$VERSION
GIT_USER,$GIT_USER 

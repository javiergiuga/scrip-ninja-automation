#!/bin/bash
# table utils
source ./scrip-ninja-automation/frontend/printTable.sh
# check version
if [ -z "$VERSION" ]
      then
...
AUTOMATION_SHORT=$(cd scrip-ninja-automation/frontend;git rev-parse --short HEAD)
AUTOMATION_BRANCH_NAME=$(cd scrip-ninja-automation/frontend;git rev-parse --abbrev-ref HEAD)
...
DOCKER_IMAGENAME,$REGISTRY/$NAMEF:$VERSION
GIT_USER,$GIT_USER 
